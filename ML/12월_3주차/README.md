...
*파이썬과 케라스로 배우는 강화학습 (이웅원외 5명)*
도서관에서 책을 뒤져보다 좋은 책을 발견한거 같아서 위 책에서 제시한 내용을 따라 공부하였다.
후에 심층 강화학습 인 액션 책을 주문해 놓았기에 위에 책을 먼저 보고 강화학습 인 액션을 보기로 계획하였다.
...
딥살사는 table형태의 강화학습이 아님 -> 경사하강법을 이용한 인공신경망으로 근사된 함수(큐함수)를 뱉어낸다.
오차함수를 먼저 정의 -> MSE
여기서 중요한 부분은 강화학습에서는 정답이 없다. 하지만 위 수식(MSE)에서는 정답 역할을 하는 부분과 예측에 해당하는 것이 나뉘어 있다.
...
장애물의 개수가 3개이므로 장애물의 위치정보, 라벨정보, 장애물 속도 까지 3*4 =12개를 가지고 기존의 에이전트의 위치(x,y)

와 도착지점 라벨까지 총 15개의 정보를 가진다. 

그러므로 input의 개수가 15개로 정해졌고 은닉층은 kears dense층 2개에 각 은닉층마다 30개의 노드를 만들었다. 그리고 출력층은 선택가능한 행동의 개수 5개로 만들었다.  
...
지금 구현한 살사코드는 ε -탐용정책에서 사용되는 ε(epsilon)을 시간에 따라서 감소시킨다.
...
추가로 책에 설명된 부분)
이번 살사의 출력은 5개의 큐함수 값을 가진다. 하지만 모델을 update하기 위해 오차함수를 계산할 출력은 이 가운데 실제로 행동이 된 하나의 큐함수 뿐이다. 따라서 타겟에서 실제 행동에 해당하는 큐함수 이외에는 예측 값에서 해당하는 큐함수 값과 동일해야 한다. 그렇기에 코드 구현에 있어서 

1. 예측을 타깃이라고 놓고
![수식 5 8](https://user-images.githubusercontent.com/38103094/103357083-ac1da900-4af5-11eb-9484-e29ff3fe5dcc.jpg)

2.실제 행동에 해당하는 부분을 뒤에서 계산(R+γQ(S,A) -- )
 
를 하여 변경하면 실제 행동에 대한 큐함수를 제외하고 타깃과 예측의 차이가 0이 된다.

(타겟에서 실제 행동에 해당하는 부분을 계산하는 코드)

![캡처](https://user-images.githubusercontent.com/38103094/103419565-eb6ff680-4bd6-11eb-97bf-e2daee0df1e0.PNG)
...
![캡처](https://user-images.githubusercontent.com/38103094/103419901-5bcb4780-4bd8-11eb-9a44-56607cae9b55.PNG)


(케라스에서 reshape함수는 아래 예제 코드를 보면 28 X 28의 행렬 형태의 데이터를 재배열(reshape)해 784 차원의 벡터로 바꾸는 함수이다)


![캡처](https://user-images.githubusercontent.com/38103094/103419982-ac42a500-4bd8-11eb-87cc-6de876b4e4f6.PNG)
...
![캡처](https://user-images.githubusercontent.com/38103094/103420132-33901880-4bd9-11eb-9c09-9812cfff2741.PNG)

입실론값이 0.01이라면 선택한 행동이 대부분 모델을 통해 나온 값으로 선택한 다는 의미이다. 에피소드의 점수가 지속적으로 낮다면 학습이 잘 안되고 있다는 의미라 한다 

장애물에 부딧히면 -1 목표지점에 도착하면 +1을 보상한다. 그래서 한 에피소드에서 가장 많이 받을 수 있는 보상은 +1값이다.

각 에피소드마다 얼마의 점수를 받았는지도 확인 할 수 있다. 
